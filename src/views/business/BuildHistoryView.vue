...
import type { BuildHistory, BuildStatus, Project } from '../../types/devops'
...
function getStatusTagColor(status: BuildStatus) {
  switch (status) {
...
function getStatusText(status: BuildStatus) {
  switch (status) {
...
function viewLog(build: BuildHistory) {
  currentLog.value = build.log || '暂无日志信息'
  showLogModal.value = true
}

async function rebuildProject(build: BuildHistory) {
  try {
    console.warn('build:', build)
    // TODO: 调用重新构建API
