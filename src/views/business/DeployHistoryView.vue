...
import type { DeployHistory, DeployStatus, Host, Project } from '../../types/devops'
...
function getStatusTagColor(status: DeployStatus) {
  switch (status) {
...
function getStatusText(status: DeployStatus) {
  switch (status) {
...
function viewLog(deploy: DeployHistory) {
  currentLog.value = deploy.log || '暂无日志信息'
  showLogModal.value = true
}

function stopContainer(deploy: DeployHistory) {
  Modal.confirm({
    title: '确认停止',
...
async function redeployProject(deploy: DeployHistory) {
  try {
    console.warn('deploy:', deploy)
    // TODO: 调用重新部署API
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      >
        ...
        <template #host="{ record }">
          <div>
            <p class="font-medium text-gray-900">
              {{ record.host?.name || '未知主机' }}
            </p>
            <p class="text-sm text-gray-500">
              {{ record.host?.ip }}:{{ record.host?.port }}
            </p>
          </div>
        </template>
        ...
        <template #status="{ record }">
          <div class="flex items-center space-x-2">
            <div class="w-2 h-2 rounded-full" :class="getStatusColor(record.status)" />
