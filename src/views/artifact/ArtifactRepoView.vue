<template>
  <div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 mb-2">制品仓库管理</h1>
        <p class="text-gray-600">管理各类制品仓库配置</p>
      </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      >
        ...
        <template #actions="{ record }">
          <a-space>
            <a-button type="text" size="small" @click="testConnection(record)" :loading="testingRepos.includes(record.id)">
              <template #icon>
                <icon-wifi />
              </template>
              测试连接
...
const editArtifactRepo = (artifactRepo: ArtifactRepo) => {
...
const deleteArtifactRepo = (artifactRepo: ArtifactRepo) => {
  Modal.confirm({
    title: '确认删除',
...
const testConnection = async (artifactRepo: ArtifactRepo) => {
  try {
