<template>
  <div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 mb-2">镜像仓库管理</h1>
        <p class="text-gray-600">管理Docker镜像仓库配置</p>
      </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
      <a-table :columns="columns" :data="imageRepos" :loading="loading" :pagination="pagination"
        @page-change="handlePageChange" @page-size-change="handlePageSizeChange">
        <template #name="{ record }">
          <div>
            <p class="font-medium text-gray-900">{{ record.name }}</p>
            <p class="text-sm text-gray-500">{{ record.url }}</p>
          </div>
        </template>

        <template #auth="{ record }">
          <div class="flex items-center space-x-2">
            <div class="w-2 h-2 rounded-full" :class="record.username ? 'bg-green-500' : 'bg-gray-400'"></div>
        ...
        <template #actions="{ record }">
          <a-space>
            <a-button type="text" size="small" @click="testConnection(record)"
              :loading="testingRepos.includes(record.id)">
              <template #icon>
                <icon-wifi />
              </template>
              测试连接
    ...
    <!-- 新建/编辑镜像仓库弹窗 -->
    <a-modal v-model:visible="showCreateModal" :title="editingImageRepo ? '编辑镜像仓库' : '添加镜像仓库'" width="600px"
...
const editImageRepo = (imageRepo: ImageRepo) => {
...
const deleteImageRepo = (imageRepo: ImageRepo) => {
  Modal.confirm({
    title: '确认删除',
...
const testConnection = async (imageRepo: ImageRepo) => {
  try {
