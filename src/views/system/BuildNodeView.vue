...
function editBuildNode(buildNode: BuildNode) {
...
function deleteBuildNode(buildNode: BuildNode) {
  Modal.confirm({
    title: '确认删除',
...
async function testConnection(buildNode: BuildNode) {
  try {
...
<template>
  <div class="p-6">
      ...
      >
        ...
        <template #status="{ record }">
          <div class="flex items-center space-x-2">
            <div class="w-2 h-2 rounded-full" :class="record.status === 'ONLINE' ? 'bg-green-500' : 'bg-red-500'" />
        ...
        <template #actions="{ record }">
          <a-space>
            <a-button type="text" size="small" :loading="testingNodes.includes(record.id)" @click="testConnection(record)">
              <template #icon>
                <IconWifi />
              </template>
              测试连接
    ...
    <a-modal
      ...
      :title="editingBuildNode ? '编辑构建节点' : '添加构建节点'"
      width="600px"
      @ok="handleSubmit"
      @cancel="resetForm"
    >
