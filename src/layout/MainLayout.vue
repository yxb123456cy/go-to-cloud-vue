<template>
  <a-layout class="min-h-screen">
    <!-- 侧边栏 -->
    <a-layout-sider v-model:collapsed="collapsed" :width="240" :collapsed-width="64" collapsible
      ...
      <div class="h-16 flex items-center justify-center border-b border-gray-200">
        <div v-if="!collapsed" class="flex items-center space-x-2">
          <div class="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center">
            <icon-cloud class="text-white text-lg" />
          </div>
      ...
      <!-- 菜单 -->
      <a-menu v-model:selected-keys="selectedKeys" v-model:open-keys="openKeys" mode="inline"
        :style="{ border: 'none', background: 'transparent' }" @menu-item-click="handleMenuClick">
        <a-menu-item key="dashboard">
          <template #icon>
            <icon-dashboard class="text-lg" />
          </template>
          控制台
        </a-menu-item>
    ...
    <a-layout :style="{ marginLeft: collapsed ? '64px' : '240px' }">
      <!-- 顶部Header -->
      <a-layout-header class="bg-white border-b border-gray-200 shadow-sm px-6 h-16 flex items-center justify-between"
        ...
        <div class="flex items-center space-x-4">
          ...
          <a-dropdown>
            <div class="flex items-center space-x-2 cursor-pointer hover:bg-gray-50 px-3 py-2 rounded-lg">
              ...
              <div class="text-sm">
                <div class="font-medium text-gray-900">{{ userStore.user?.username || 'Admin' }}</div>
                <div class="text-gray-500">管理员</div>
              </div>
